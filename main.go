...
func printLsL(tmp os.FileInfo, Name, path string) {
	...
	if now.Before(date) {
		if mode[0] != 'c' {
			...
			fmt.Printf(printFormat, mode, stat.Nlink, user, group, data.CountDivision(stat.Rdev), data.CountMod(stat.Rdev), month, day, date.Year(), s)
		}
	} else {
		if mode[0] != 'c' {
...
func Parse(path string, cut_path int) {
	...
	dir2, _ := tmp_f.Stat()
	arr = append(arr, data.Files{dir, "."})   // <- parent directory
	arr = append(arr, data.Files{dir2, ".."}) // <- out directory

	n := len(tmp_arr)

	for i := 0; i < n; i++ { // <- Using our structure for sorting files by name or date
		arr = append(arr, data.Files{tmp_arr[i], tmp_arr[i].Name()})
	}
...
// https://askubuntu.com/questions/17299/what-do-the-different-colors-mean-in-ls - use this to read about colors in ls
// https://golangbyexample.com/print-output-text-color-console/ - how to use output bash colors
// https://www.geeksforgeeks.org/introsort-or-introspective-sort/ - best sort algo
//But we will use this sort - https://ru.wikipedia.org/wiki/%D0%9F%D0%B8%D1%80%D0%B0%D0%BC%D0%B8%D0%B4%D0%B0%D0%BB%D1%8C%D0%BD%D0%B0%D1%8F_%D1%81%D0%BE%D1%80%D1%82%D0%B8%D1%80%D0%BE%D0%B2%D0%BA%D0%B0

/*
Use this code to test color
for i := 0; i < 256; i++ {
		fmt.Print(i, " -> ")
		fmt.Printf(format, i, "test\n")
}
*/
